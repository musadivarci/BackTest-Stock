...
function technicalSignal(bars: Bar[]): Technical {
  ...
  if (Number.isFinite(rsi14)) {
    ...
    else if (rsi14 < 45 && rsi14 >= 30) score -= 1;
...
export async function GET(req: NextRequest) {
  ...
  try {
    const allBars = await getBars(symbol);
    const bars = allBars.slice(-tradingDays);
    const best = optimize(bars);
    const technical = technicalSignal(allBars.slice(-100));
    const last = bars[bars.length - 1];
    return NextResponse.json({
      symbol, months, tradingDays, bars: bars.length, lastPrice: last.close, lastDate: last.date, best, technical,
      ...
      note: 'Teknik görünüm RSI, MACD ve hareketli ortalamalardan uygulama içinde hesaplanır; Investing/TradingView sinyali değildir.'
    });
  } catch (error) {
