...
export default function Home() {
  ...
  return <main><header><div><p className="eyebrow">BIST · SWING STRATEGY</p><h1>BackTest Stock</h1><p className="sub">Backtest oranı + güncel teknik görünüm.</p></div><div className="period"><span>BACKTEST</span><select value={months} onChange={e => setMonths(Number(e.target.value))}><option value={1}>1 ay · ~21 seans</option><option value={2}>2 ay · ~42 seans</option><option value={3}>3 ay · ~63 seans</option><option value={6}>6 ay · ~126 seans</option></select></div></header>
    <div className="grid"><StockCard symbol="ASELS" months={months} /><StockCard symbol="THYAO" months={months} /></div>
    <footer><span>Teknik görünüm: RSI + MACD + SMA20/SMA50 + EMA20.</span><span>Geçmiş performans · yatırım tavsiyesi değildir.</span></footer>
  </main>;
}
